...
public static class ResultOverrides
{
    ...
    {
        if (result.IsFailed) throw new NotImplementedException("A Result Error Was Not Handled");
    }
    ...
    public static bool HasValidationError<T>(this Result<T> result, out ValidationProblem typedResults)
    {
        typedResults = TypedResults.ValidationProblem(GetValidationFailure(result.Errors));
        return result.HasError<FluentValidationFailure>();
    }
    
    public static bool HasBeenDeletedAlready(this Result result, out StatusCodeHttpResult typedResults)
    {
        typedResults = TypedResults.StatusCode(410);
        return result.HasError<AlreadyDeletedFailure>();
    }
    
    public static bool HasBeenDeletedAlready<T>(this Result<T> result, out StatusCodeHttpResult typedResults)
    {
        typedResults = TypedResults.StatusCode(410);
        return result.HasError<AlreadyDeletedFailure>();
    }
