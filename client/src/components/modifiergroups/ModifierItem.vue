  ...
  <div v-else class="d-flex flex-column flex-md-row align-self-center justify-content-between m-2">
    <div class="align-self-center">
      ...
      <Tag v-if="props.modifier?.targetExpression" severity="info" class="ml-2">
        <span>{{ props.modifier.targetExpression.name }}</span><span v-if="props.modifier.targetProgressionPath"> - {{ props.modifier.targetProgressionPath.name }}</span><span v-if="props.modifier.targetProgressionLevel"> - {{
          props.modifier?.targetProgressionLevel?.name
        }}</span>
      </Tag>
    </div>
    <div
