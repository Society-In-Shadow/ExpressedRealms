...
{
    public static async Task<Ok<GetKnowledgeResponse>> GetKnowledge(int id, IGetEditKnowledgeUseCase createKnowledgeUseCase)
    {
        var results = await createKnowledgeUseCase.ExecuteAsync(new GetEditKnowledgeModel()
        {
            Id = id,
        });

        return TypedResults.Ok(new GetKnowledgeResponse()
        {
                Id = results.Value.Id,
