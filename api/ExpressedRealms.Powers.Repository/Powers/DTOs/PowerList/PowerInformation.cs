...
{
    public static readonly Expression<Func<Power, PowerInformation>> Selector = x => new PowerInformation
    {
        Id = x.Id,
        Name = x.Name,
        Category = x.CategoryMappings
            .Select(y => new DetailedInformation(
                y.Category.Name,
        ...
        Prerequisites = x.Prerequisite != null
            ? new PrerequisiteDetails
            {
                RequiredAmount = x.Prerequisite.RequiredAmount,
                Powers = x.Prerequisite.PrerequisitePowers
                    .Select(pp => pp.Power.Name)
                    .ToList()
            }
            : null
    };
