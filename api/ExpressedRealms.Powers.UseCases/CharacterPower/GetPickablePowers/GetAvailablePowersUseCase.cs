...
{
    public async Task<Result<List<PowerPathReturnModel>>> ExecuteAsync(GetAvailablePowersModel model)
    {
        ...
        return Result.Ok(powers.Value.Select(x => new PowerPathReturnModel()
        {
            Name = x.Name,
            Powers = x
