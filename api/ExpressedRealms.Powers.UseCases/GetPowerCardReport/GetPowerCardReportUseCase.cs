...
public class GetPowerCardReportUseCase(IPowerPathRepository repository, IExpressionRepository expressionRepository) : IGetPowerCardReportUseCase
{
    public async Task<MemoryStream> ExecuteAsync(GetPowerCardReportUseCaseModel model)
    {
        ...
        var report = PowerCardReport.GenerateReport(
            data.Value.SelectMany(x => x.Powers.Select(y => new PowerCardData()
                {
