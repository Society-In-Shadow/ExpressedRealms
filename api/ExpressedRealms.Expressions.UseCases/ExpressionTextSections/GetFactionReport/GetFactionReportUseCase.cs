...
) : IGetFactionReportUseCase
{
    ...
    public async Task<Result<MemoryStream>> ExecuteAsync(GetFactionReportModel model)
    {
        ...
        var factions = await factionsUseCase.ExecuteAsync(new GetFactionsModel()
        {
        ...
        var reportData = factions.Value.Factions.Select(x => new FactionData()
        {
            Name = x.Name,
            Background = x.Background,
            FactionLevels = x.FactionLevels.Select(y => new FactionLevelData()
            {
                RankName = y.RankName,
                KnowledgeLevel = y.KnowledgeLevel,
                KnowledgeName = y.Knowledge,
                KnowledgeSpecialization = y.Specialization,
                Power = y.Power is null ? null : new PowerData()
                {
