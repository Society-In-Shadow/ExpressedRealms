...
{
    private async Task AddUpdateResourcesAndPermissions(IReadOnlyList<PermissionResource> codeSidePermissions)
    {
        ...
        await strategy.ExecuteAsync(async () =>
        {
            ...
            var resources = await context.Set<PermissionResource>()
                .Include(x => x.Permissions)
                .ToListAsync();

            var dbResourceIds = resources.Select(x => x.Id).ToHashSet();
            var missingResources = codeSidePermissions.Where(x => !dbResourceIds.Contains(x.Id)).ToList();
            
            if (missingResources.Count != 0)
            {
                // GPT was very insistent on this
                var cleanedResources = missingResources.Select(x => new PermissionResource()
                {
                    Name = x.Name,
                    Id = x.Id,
            ...
            foreach (var resource in resources)
            {
                ...
                if (codeSideResource is null) continue;
                ...
                var missingPermissions = codeSideResource.Permissions.Where(x => !dbResourcePermissionIds.Contains(x.Id)).ToList();
                ...
                {
                    var codeSidePermission = codeSideResource.Permissions.FirstOrDefault(x => x.Id == permission.Id);
                    
                    // Deleted resources will be handled outside of this loop
                    // There's a bit more to it then meets the eye
                    if (codeSidePermission is null) continue;
    ...
    private async Task RemoveResourceAndPermissions(IReadOnlyList<PermissionResource> codeSidePermissions)
    {
        ...
        await context.Set<RolePermissionMapping>()
        ...
        await context.Set<PermissionResource>().Where(x => x.Permissions.Count == 0)
            .ExecuteDeleteAsync();
