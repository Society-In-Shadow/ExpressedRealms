...
public class EditExpressionHierarchyDtoValidator : AbstractValidator<EditExpressionHierarchyDto>
{
    public EditExpressionHierarchyDtoValidator(ExpressedRealmsDbContext dbContext)
    {
        ...
        RuleFor(x => x)
            .MustAsync(async (dto, cancellationToken) =>
            {
                ...
                return dto.Items.Select(x => x.Id).OrderBy(id => id)
                    .SequenceEqual(expressionSections.Select(x => x.Id).OrderBy(id => id));
            })
            .WithMessage("There has been changes made to the expression that are not reflected in the UI. Please refresh the page and try again.");
        RuleFor(x => x)
            .Must((dto, cancellationToken) =>
            {
        ...
        RuleFor(x => x)
            .Must((dto, cancellationToken) =>
            {
                ...
                foreach (var group in groups)
                {
                    ...
                    {
                        if (item.SortOrder == startingCount) return true;
                        startingCount++;
                    }
                }
    ...
    private bool HasCycles(IEnumerable<EditExpressionHierarchyItemDto> nodes)
    {
        ...
        {
            if (path.Contains(id)) return true; // Cycle detected
            if (!nodeLookup.ContainsKey(id) || nodeLookup[id] == null) return false; // Reached root
        
            path.Add(id); // Add current node to the path
            var parentId = nodeLookup[id]; // Get parent
            if (parentId.HasValue && DetectCycle(parentId.Value, visited, path)) return true;
        ...
        foreach (var node in nodes)
        {
            if (!visited.Contains(node.Id))
            {
                ...
                if (DetectCycle(node.Id, visited, path)) return true; // Cycle found
            }
        }
