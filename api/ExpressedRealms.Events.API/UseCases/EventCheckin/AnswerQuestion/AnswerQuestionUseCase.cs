...
) : IAnswerQuestionUseCase
{
    ...
    )
    {
        ...
        var questionResponse = await checkinRepository.GetCheckinQuestionResponseAsync(checkin!.Id, model.QuestionId);

        if (questionResponse is null)
        {
            await checkinRepository.AddCheckinQuestionResponseAsync(new CheckinQuestionResponse()
            {
