...
internal sealed class DiscordService(IKeyVaultManager keyVaultManager, ILogger<DiscordService> logger, IHostEnvironment environment) : IDiscordService
{
    ...
    private async Task<bool> SetupDiscordClient()
    {
        ...
        {
            logger.LogWarning("No Discord Token provided. Discord Integration will be disabled.  Actions will be treated as successful.");
            return false;
        }
        
        if (_discordRestClient.LoginState == LoginState.LoggedOut)
        {
            await _discordRestClient.LoginAsync(TokenType.Bot, await keyVaultManager.GetSecret(DiscordSettings.DiscordBotToken));
        }

        return true;
    }

    public async Task<Result> SendMessageToChannelAsync(DiscordChannel targetChannel, string message, Embed[]? embeds = null)
    {
        ...
        if (await _discordRestClient.GetChannelAsync((ulong)targetChannel) is not IMessageChannel channel)
        {
