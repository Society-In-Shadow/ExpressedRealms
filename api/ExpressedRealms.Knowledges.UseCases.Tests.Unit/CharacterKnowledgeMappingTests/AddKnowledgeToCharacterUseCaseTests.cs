...
public class AddKnowledgeToCharacterUseCaseTests
{
    ...
    public AddKnowledgeToCharacterUseCaseTests()
    {
        ...
        A.CallTo(() => _levelRepository.KnowledgeLevelExists(_model.KnowledgeLevelId)).Returns(true);
        A.CallTo(() => _mappingRepository.MappingAlreadyExists(_model.KnowledgeId, _model.CharacterId)).Returns(false);
        A.CallTo(() => _mappingRepository.GetExperienceSpentOnKnowledgesForCharacter(_model.CharacterId)).Returns(0);
        A.CallTo(() => _knowledgeRepository.GetKnowledgeAsync(_model.KnowledgeId)).Returns(new Knowledge()
        {
            KnowledgeTypeId = 3
        });
        A.CallTo(() => _levelRepository.GetKnowledgeLevel(_model.KnowledgeLevelId)).Returns(
        ...
        var validator = new AddModelValidator(_knowledgeRepository, _characterRepository, _mappingRepository,
            _levelRepository);

        _useCase = new AddKnowledgeToCharacterUseCase(_mappingRepository, _levelRepository, _knowledgeRepository, validator, CancellationToken.None);
    }
    ...
    public async Task ValidationFor_KnowledgeId_WillFail_WhenItDoesNotExist()
    {
        ...
        result.MustHaveValidationError(nameof(AddModel.KnowledgeId), "The Knowledge does not exist.");
    }
    ...
    public async Task ValidationFor_CharacterId_WillFail_WhenItDoesNotExist()
    {
        ...
        result.MustHaveValidationError(nameof(AddModel.CharacterId), "The Character does not exist.");
    }
    ...
    public async Task ValidationFor_KnowledgeLevelId_WillFail_WhenItsEmpty()
    {
        ...
        result.MustHaveValidationError(nameof(AddModel.KnowledgeLevelId), "Knowledge Level Id is required.");
    }
    
    [Fact]
    public async Task ValidationFor_KnowledgeLevelId_WillFail_WhenItDoesNotExist()
    {
        A.CallTo(() => _levelRepository.KnowledgeLevelExists(_model.KnowledgeLevelId)).Returns(false);
        var result = await _useCase.Execute(_model);

        result.MustHaveValidationError(nameof(AddModel.KnowledgeLevelId), "The Knowledge Level does not exist.");
    }
    
    [Fact]
    public async Task ValidationFor_DuplicateMapping_WillFail_WhenItDoesExist()
    {
        A.CallTo(() => _mappingRepository.MappingAlreadyExists(_model.KnowledgeId, _model.CharacterId)).Returns(true);
        var result = await _useCase.Execute(_model);

        result.MustHaveValidationError(nameof(AddModel.KnowledgeId), "The knowledge already exists for this character.");
    }
    ...
    public async Task ValidationFor_Notes_WillFail_WhenMaxLengthIsGreaterThan5000()
    {
        ...
        var result = await _useCase.Execute(_model);
        result.MustHaveValidationError(nameof(AddModel.Notes), "Notes must be less than 5000 characters.");
    }
    ...
    public async Task UseCase_GetsKnowledgeLevel()
    {
        ...
        A.CallTo(() => _levelRepository.GetKnowledgeLevel(_model.KnowledgeLevelId)).MustHaveHappenedOnceExactly();
    }
    ...
    public async Task UseCase_GetsKnowledge()
    {
        ...
        A.CallTo(() => _knowledgeRepository.GetKnowledgeAsync(_model.KnowledgeId)).MustHaveHappenedOnceExactly();
    }

    [Theory]
    [InlineData(2, 2)]
    [InlineData(3, 4)]
    public async Task UseCase_GrabsCorrectXp_DependingOnKnowledgeType(int knowledgeTypeId, int xpAmount)
    {
        A.CallTo(() => _knowledgeRepository.GetKnowledgeAsync(_model.KnowledgeId)).Returns(new Knowledge()
        {
        ...
        A.CallTo(() => _mappingRepository.GetExperienceSpentOnKnowledgesForCharacter(_model.CharacterId)).Returns(100);
    ...
    public async Task UseCase_CalculatesAvailableXP_Correctly( int xpAmount)
    {
        ...
        A.CallTo(() => _mappingRepository.GetExperienceSpentOnKnowledgesForCharacter(_model.CharacterId)).Returns(xpAmount);
    ...
    {
        A.CallTo(() => _mappingRepository.GetExperienceSpentOnKnowledgesForCharacter(_model.CharacterId)).Returns(7);
    ...
    public async Task UseCase_AddCharacterKnowledgeMapping_WhenItHasEnoughXp()
    {
        ...
        A.CallTo(() => _mappingRepository.AddCharacterKnowledgeMapping(
            A<CharacterKnowledgeMapping>.That.Matches(
                x => 
    ...
    {
        A.CallTo(() => _mappingRepository.AddCharacterKnowledgeMapping(A<CharacterKnowledgeMapping>._)).Returns(5);
        var result = await _useCase.Execute(_model);
