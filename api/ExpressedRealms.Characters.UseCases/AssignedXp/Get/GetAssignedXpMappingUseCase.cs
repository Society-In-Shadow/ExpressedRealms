...
{
    public async Task<Result<List<XpMappingInfoReturnModel>>> ExecuteAsync(
        GetAssignedXpMappingModel model
    )
    {
        ...
        else
        {
            ...
            mappings.AddRange(
                await mappingRepository.GetAllPlayerMappingsAsync(character!.PlayerId)
            );
            var events = await eventRepository.GetEventsWithAvailableXp();
            mappings.AddRange(
                events.Select(x => new XpMappingInfoDto()
                {
                    Id = -1,
