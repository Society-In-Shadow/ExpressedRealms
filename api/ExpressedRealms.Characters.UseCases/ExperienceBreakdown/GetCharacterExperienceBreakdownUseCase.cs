...
) : IGetCharacterExperienceBreakdownUseCase
{
    ...
    )
    {
        ...
        {
            var advantageXp = await blessingRepository.GetExperienceSpentOnBlessingsForCharacter(model.CharacterId);
            costs.Add(new ExperienceTotalMax("Advantage XP" , advantageXp, -1, true, false));
        
            var disadvantageXp = await blessingRepository.GetExperienceAvailableToSpendOnCharacter(model.CharacterId);
