...
public class ConfirmedUserInfoUseCaseTests
{
    ...
    public ConfirmedUserInfoUseCaseTests()
    {
        ...
        A.CallTo(() => _eventCheckinRepository.GetCheckinAsync(EventId, PlayerId)).Returns(new Checkin { Id = CheckinId });
        ...
        A.CallTo(() => _eventCheckinRepository.GetAnsweredQuestions(CheckinId)).Returns(new List<CheckinQuestionResponse>());
    ...
    public async Task UseCase_WillNotCreateCheckin_IfItDoesExist()
    {
        ...
        A.CallTo(() => _eventCheckinRepository.CreateCheckinAsync(A<Checkin>._)).MustNotHaveHappened();
    }
    
    [Fact]
    public async Task UseCase_WillCreateCheckin_IfItDoesNotExist()
    {
        A.CallTo(() => _eventCheckinRepository.GetCheckinAsync(EventId, PlayerId)).Returns(Task.FromResult<Checkin?>(null));
        
        await _useCase.ExecuteAsync(_model);
        
        A.CallTo(() => _eventCheckinRepository.CreateCheckinAsync(A<Checkin>.That.Matches(x => x.EventId == EventId && x.PlayerId == PlayerId))).MustHaveHappenedOnceExactly();
    }
    ...
    public async Task UseCase_WillReturn_QuestionAnswers()
    {
        ...
        Assert.Equivalent(list.Select(x => new QuestionResponse()
        {
            Response = x.Response,
