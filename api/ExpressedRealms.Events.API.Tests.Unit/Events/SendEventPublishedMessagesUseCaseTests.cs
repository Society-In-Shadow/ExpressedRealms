...
public class SendEventPublishedMessagesUseCaseTests
{
    ...
    {
        _model = new SendEventPublishedMessagesModel()
        {
            Id = 2,
        };
        ...
        _useCase = new SendEventPublishedMessagesUseCase(_repository, validator, _discordService, CancellationToken.None);
    }
    ...
    public async Task ValidationFor_Id_WillFail_WhenEmpty()
    {
        ...
        results.MustHaveValidationError(nameof(SendEventPublishedMessagesModel.Id), "Id is required.");
    }
    ...
    public async Task UseCase_MessageContains_EventTitle_WithFormatting()
    {
        ...
        A.CallTo(() => _discordService.SendMessageToChannelAsync(
    ...
    public async Task UseCase_MessageContains_LocationMessage_WithFormatting()
    {
        ...
        A.CallTo(() => _discordService.SendMessageToChannelAsync(
            DiscordChannel.PublicAnnouncements, 
            A<string>.That.Contains("We are happy to announce that we will be attending **Sioux City Geek Con** out in the **Sioux City Convention Center Sioux City, Iowa**!"),
            A<Embed[]>._
        )).MustHaveHappenedOnceExactly();
    }
    ...
    public async Task UseCase_MessageContains_ConDatesMessage_WithFormatting()
    {
        ...
        A.CallTo(() => _discordService.SendMessageToChannelAsync(
            DiscordChannel.PublicAnnouncements, 
            A<string>.That.Contains("Their scheduled dates are **Friday August 22, 2025** to **Sunday August 24, 2025**."),
            A<Embed[]>._
        )).MustHaveHappenedOnceExactly();
    }
    ...
    public async Task UseCase_ContainsMessage_ThatWeWillAttendASubSetOfDays_WithFormatting()
    {
        ...
        A.CallTo(() => _discordService.SendMessageToChannelAsync(
            DiscordChannel.PublicAnnouncements, 
            A<string>.That.Contains("# Society in Shadows will be there on Friday, Saturday and Sunday!"),
            A<Embed[]>._
        )).MustHaveHappenedOnceExactly();
    }
    ...
    public async Task UseCase_ContainsMessage_OfHowMuchXPTheyWillEarn_WithFormatting()
    {
        ...
        A.CallTo(() => _discordService.SendMessageToChannelAsync(
            DiscordChannel.PublicAnnouncements, 
            A<string>.That.Contains("Once the event starts, everyone will have access to an additional 32 XP for this event."),
            A<Embed[]>._
        )).MustHaveHappenedOnceExactly();
    }
    ...
    public async Task UseCase_ContainsMessage_StatingTheTimezone_WithFormatting()
    {
        ...
        A.CallTo(() => _discordService.SendMessageToChannelAsync(
            DiscordChannel.PublicAnnouncements, 
            A<string>.That.Contains("Sioux City Geek Con is in the Central Standard Time and our schedule below reflects that."),
            A<Embed[]>._
        )).MustHaveHappenedOnceExactly();
    }
    ...
    public async Task UseCase_ContainsMessage_SaturdaysScheduledEvent_WithFormatting()
    {
        ...
        A.CallTo(() => _discordService.SendMessageToChannelAsync(
            DiscordChannel.PublicAnnouncements, 
            A<string>.That.Contains("Saturday\n\n**11:00 AM** - 11:30 AM - **Test Event 2**"),
            A<Embed[]>._
        )).MustHaveHappenedOnceExactly();
    }
    ...
    public async Task UseCase_PassesThrough_WebsiteLinkAsEmbed()
    {
        ...
        A.CallTo(() => _discordService.SendMessageToChannelAsync(
            DiscordChannel.PublicAnnouncements, 
            A<string>._,
            A<Embed[]>.That.Matches(embeds => embeds.Any(k => k.Url == _dbModel.WebsiteUrl && k.Title == _dbModel.WebsiteName))
        )).MustHaveHappenedOnceExactly();
    }
    ...
    {
        var mapUrl = $"https://www.google.com/maps/search/?api=1&query={WebUtility.UrlEncode(_dbModel.Location)}";
        await _useCase.ExecuteAsync(_model);
        A.CallTo(() => _discordService.SendMessageToChannelAsync(
            DiscordChannel.PublicAnnouncements, 
            A<string>._,
            A<Embed[]>.That.Matches(embeds => embeds.Any(k => k.Url == mapUrl && k.Title == _dbModel.Location))
        )).MustHaveHappenedOnceExactly();
    }
}
