using ExpressedRealms.Blessings.UseCases.BlessingLevels.CreateBlessingLevel;
using ExpressedRealms.Server.Shared;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Http.HttpResults;
using Microsoft.AspNetCore.Mvc;

namespace ExpressedRealms.Blessings.API.BlessingLevels.CreateBlessingLevel;

public static class CreateBlessingLevelEndpoint
{
    public static async Task<Results<Ok<int>, NotFound, ValidationProblem>> Execute(
        int blessingId,
        [FromBody] CreateBlessingLevelRequest request,
        [FromServices] ICreateBlessingLevelUseCase useCase
    )
    {
        var results = await useCase.ExecuteAsync(
            new CreateBlessingLevelModel()
            {
                Level = request.Level,
                Description = request.Description,
                BlessingId = blessingId,
                XpCost = request.XpCost,
                XpGain = request.XpGain
            }
        );

        if (results.HasValidationError(out var validationProblem))
            return validationProblem;
        if (results.HasNotFound(out var notFound))
            return notFound;

        results.ThrowIfErrorNotHandled();

        return TypedResults.Ok(results.Value);
    }
}
