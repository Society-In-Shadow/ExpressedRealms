...
public static class CrudGenerator
{
    ...
    private static void Generate(
        GenerateUseCasesModel generationModel,
        string templateRoot,
        string baseFolderName
    )
    {
        var outputSource = string.IsNullOrWhiteSpace(generationModel.TargetFolder)
            ? Environment.CurrentDirectory
            : Path.GetFullPath(generationModel.TargetFolder);
    ...
    private static void GenerateDirectory(string templateDir, string outputDir, object model)
    {
        ...
        foreach (var directory in Directory.GetDirectories(templateDir))
        {
            ...
            GenerateDirectory(directory, Path.Combine(outputDir, dirName), model);
        }

        foreach (var file in Directory.GetFiles(templateDir))
        {
            var fileName = Transform(Path.GetFileName(file), model).Replace(".sbn", ".cs");
            ...
            File.WriteAllText(Path.Combine(outputDir, fileName), result);
    ...
    private static string Transform(string input, object model)
    {
        ...
        return input.Replace("Entity", singular).Replace("Entities", plural);
    }

    private static string Get(object model, string prop)
    {
        return model.GetType().GetProperty(prop)!.GetValue(model)!.ToString()!;
    }
}
